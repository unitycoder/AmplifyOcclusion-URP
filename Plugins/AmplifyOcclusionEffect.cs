...
public class AmplifyOcclusionEffect : MonoBehaviour
{
    ...
    void OnEnable()
    {
        ...
        if (GraphicsSettings.HasShaderDefine(BuiltinShaderDefine.SHADER_API_MOBILE))
        {
    ...
    {
        Reset();
        if (m_nextID <= 1) // crude; or track active instance count
        {
            if (m_occlusionMat) DestroyImmediate(m_occlusionMat);
            if (m_blurMat) DestroyImmediate(m_blurMat);
            if (m_applyOcclusionMat) DestroyImmediate(m_applyOcclusionMat);
        }
    }
    ...
    void OnPreRender()
    {
        ...
        if (m_targetCamera != null)
        {
            ...
            if ((m_targetCamera.cameraType == UnityEngine.CameraType.SceneView) &&
                (((PerPixelNormals == PerPixelNormalSource.GBuffer) && m_targetCamera.orthographic) ||
                  ((PerPixelNormals == PerPixelNormalSource.Camera) && sv != null && !sv.sceneLighting)))
            {
